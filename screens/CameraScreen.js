...
const deviceLength = Dimensions.get('window').length
...
export default class CameraScreen extends React.Component {
  ...
  toggleView = () => this.setState({ showPhoto: !this.state.showPhoto, newPhotos: false });
  ...
  takePicture = () => {
    if (this.camera) {
      this.camera.takePictureAsync({quality: 0}).then(data => ImageManipulator.manipulate(data.uri, [{crop: {
        originX: 0,
        originY: (data.height - data.width) / 2,
        width: data.width,
        height: data.width
      }}], {base64: true})).then(data => {
        this.uri = data.uri
  ...
  renderPhoto() {
    return <PhotoScreen navigation={this.props.navigation} uri={this.uri} base64={this.base64} onPress={this.toggleView.bind(this)} />;
  }
  renderNoPermissions = () =>
  ...
  renderCamera = () =>
    (
      <View style={{ flex: 1, backgroundColor: 'rgba(20,20,20, .5)', }}>
        <Camera
          ref={ref => {
            this.camera = ref;
          }}
          ...
          <View style={{position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', flex: 1,}}>
            <View style={{backgroundColor: 'rgba(0,0,0, 0.5)', flex: 1, flexGrow: 1, width: deviceWidth}}></View>
            <View style={[styles.cropView]}>
              <Ionicons name="ios-qr-scanner-outline" size={deviceWidth+50} color="white"></Ionicons>
            </View>
            <View style={{backgroundColor: 'rgba(0,0,0, 0.5)', flex: 1, flexGrow: 1, width: deviceWidth}}></View>
          </View>
  ...
  render() {
    ...
    const content = this.state.showPhoto ? this.renderPhoto() : cameraScreenContent;
